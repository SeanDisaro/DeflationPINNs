...
from src.lossFunctions.AllenCahnPINNLoss import pimlLoss_w_AutoGrad_allenCahn
...
def defPINNLossPIML_w_AD_LDG( x: torch.Tensor, modelOut:list[list[torch.Tensor]], boundaryPoints:torch.Tensor = None,modelOutBoundary:list[list[torch.Tensor]] = None ,
    ...
    deflationLossOut = linearDeflationLoss_dictModel(modelOut = modelOut, maxLoss = deflationLossPoints[0], maxDistance = deflationLossPoints[1], keyOutModel="out1", keyOutModel2="out2")
...
def defPINNLossPIML_w_AD_allenCahn( x: torch.Tensor, modelOut:list[list[torch.Tensor]], boundaryPoints:torch.Tensor = None, modelOutBoundary:list[list[torch.Tensor]] = None ,
                     deflationLossPoints: tuple[float,float] = (10000.,1.) , alpha:float = 1., beta:float = 0.1, delta: float = 1., omega = 6) -> torch.Tensor:

    loss_PDEAndBoundary = pimlLoss_w_AutoGrad_allenCahn(  modelOut = modelOut,x=x, boundaryPoints = boundaryPoints, modelOutBoundary = modelOutBoundary ,
                                    alpha = alpha, beta = beta, omega = omega)
